...
def is_report_safe(report: list[int]):
    ...
    if not is_difference_safe(report[0] - report[1]):
        # print("first difference not safe", report)
        return False

    for j in range(0, len(report) - 1):
        difference = report[j+1] - report[j]
        if is_difference_safe(difference) and is_direction_safe(difference, is_increasing):
            # print("difference safe", report, is_difference_safe(difference), is_direction_safe(difference, is_increasing))
            continue
        else:
            # print("difference not safe", report)
            return False
...
def calculate_num_of_safe_reports_with_dampener(reports: list[list[int]]):
    ...
    for _, report in enumerate(reports):
        if is_report_safe(report):
            print("report safe from start", report)
            num_of_safe_reports += 1
        else:
            for i, _ in enumerate(report):
                ...
                if is_report_safe(safe_report):
                    print("report safe after removing", report, safe_report)
                    num_of_safe_reports += 1
                    break
