...
def do_page_numbers_obey_rules(order_dict: dict[int, dict[str, list[int]]], page_numbers: list[int]):
...
def get_middle_sum_of_correctly_ordered_pages(page_ordering_rules: list[list[int]], pages_to_produce: list[list[int]]):
